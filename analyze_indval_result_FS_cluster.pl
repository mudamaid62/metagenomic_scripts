...
my $analyzed_out = shift(@ARGV);
my $by_protein_result = shift(@ARGV);
...
while(my $x = <DATA>){
	...
	my $abundance = 0;
	if($mode eq "SARG"){
		$metadata = "$array[3]\t$array[4]\t$array[5]\t$array[6]\t$array[7]\t$array[8]\t$array[9]";
		$counts = $array[10];
		$abundance = $array[11];
		$taxonomy = $array[12];
	}elsif($mode eq "VFDB"){
		$metadata = "$array[3]\t$array[4]\t$array[5]\t$array[6]\t$array[7]\t$array[8]";
		$counts = $array[9];
		$abundance = $array[10];
		$taxonomy = $array[11];
	}
...
close DATA;
open(INDVAL,"$indval");
open(ANALYZED_OUT, ">$analyzed_out");
open(PROTEIN_OUT, ">$by_protein_result");
while(my $x = <INDVAL>){
	chomp($x);
	my($FS_cluster,$a_value,$b_value,$stat,$p_value,$sig,$group) = split(/\t/,$x);
	my @proteins;
	foreach my $z(keys %clusters){
		if($clusters{$z} eq "$FS_cluster"){
			push @proteins,$z;
		}
	}
	...
	foreach my $n(@names){
		...
		foreach my $p(@proteins){
			my $name = "$n~$p";
			if(exists($site_protein_tax{$name})){
				if(!exists($cluster_number_of_proteins{$clusters{$p}}) and !exists($used_proteins{$p})){
					$cluster_number_of_proteins{$clusters{$p}} = 1;
					$used_proteins{$p} = "used";
				}elsif(!exists($used_proteins{$p})){
					$cluster_number_of_proteins{$clusters{$p}}++;
					$used_proteins{$p} = "used";
				}
				...
				$n_abundance += $site_protein_abundance{$name};
				$n_counts += $site_protein_counts{$name};
				print PROTEIN_OUT "$p\t$clusters{$p}\t$n\t$site_protein_tax{$name}\t$site_protein_counts{$name}\t$site_protein_abundance{$name}\t$cluster_metadata{$clusters{$p}}\n";
			}
		}
		push @abundances, $n_abundance;
		push @counts, $n_counts;
	}
	my $taxonomy = solve_tax($tax_to_solve);
	my $counts_string = join "\t",@counts;
	my $abundances_string = join "\t",@abundances;
	print ANALYZED_OUT "$x\t$cluster_metadata{$FS_cluster}\t$cluster_number_of_proteins{$FS_cluster}\t$taxonomy\t$counts_string\t$abundances_string\n";
}
