...
my $result = shift(@ARGV); #annopro concatenated result
...
while(my $z = <RESULT>){
	...
	my @array = split(/\,/,$z);
	if(!exists($clusters{$array[0]})){
		next;
	}
	if(!exists($temp{$clusters{$array[0]}})){
		$temp{$clusters{$array[0]}} = $z;
	}else{
		$temp{$clusters{$array[0]}} .= "~$z";
	}
}
close RESULT;
print "Result file loaded into RAM\n";
foreach my $r(sort keys %clusters_list){
	...
	my @r_array = split(/~/,$temp{$r});
	foreach my $q(@r_array){
		print R_TEMP "$q\n";
	}
	close R_TEMP;
}
