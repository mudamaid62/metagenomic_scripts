...
while(my $x = <DATA>){
	...
	$site_protein_scaled{$name} = $scaled;
}
close DATA;
open(INDVAL,"$indval");
while(my $x = <INDVAL>){
	...
	foreach my $n(@names){
		...
		if(exists($site_protein_tax{$name})){
			...
			push @scaleds, $site_protein_scaled{$name};
		}else{
			push @abundances, $default;
			push @counts, $default;
			push @scaleds, $default;
		}
	}
	my $taxonomy = solve_tax($tax_to_solve);
	my $counts_string = join "\t",@counts;
	my $abundances_string = join "\t",@abundances;
	my $scaled_string = join "\t",@scaleds;
	print "$x\t$protein_metadata{$protein}\t$taxonomy\t$counts_string\t$abundances_string\t$scaled_string\n";
}
